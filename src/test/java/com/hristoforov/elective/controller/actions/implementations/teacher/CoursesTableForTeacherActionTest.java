...
class CoursesTableForTeacherActionTest {
    ...
    void testExecuteDoGet() throws ServletException, IOException {
        ...
        when(request.getRequestDispatcher(TABLE_FOR_TEACHER_PAGE)).thenReturn(rd);
        new CoursesTableForTeacherAction(appContext).executeDoGet(request, response);
    }
    ...
    void testExecuteDoPost() throws ServletException, IOException {
        ...
        when(appContext.getCourseDao()).thenReturn(courseDao);
        when(appContext.getUserDao()).thenReturn(userDao);
        new CoursesTableForTeacherAction(appContext).executeDoPost(request, response);
    }
}